...
# Try to import official Google Generative AI client
_HAS_GOOGLE_CLIENT = False
try:
    import google.generativeai as genai
    _HAS_GOOGLE_CLIENT = True
...
# ---------------------------
API_KEY = os.getenv("GOOGLE_API_KEY")
MODEL = os.environ.get('GEN_MODEL', 'gemini-2.5-flash')  # Use 2.5-flash
...
def _build_prompt(transcript: str, topic: str) -> str:
    header = (
        "You are a study-pack generator.\n"
        "RETURN ONLY a single VALID JSON object. No text outside JSON.\n"
        "Schema: {summary:string, flashcards:[{q,a}], quiz:[{question,options,answer,explanation}], study_plan:object}\n\n"
    )
    prompt = header
    prompt += f"TRANSCRIPT:\n{transcript}\n\nTOPIC: {topic}\n\n"
    prompt += "RETURN a single valid JSON object matching the schema above."
...
def generate_materials(transcript: str, topic: str):
    ...
    def _attempt_correction(prev_text: str, attempts: int = 2):
        for _ in range(attempts):
            followup = (
                ...
                "{summary, flashcards, quiz, study_plan}\n"
            )
            try:
                corrected = call_gemini(followup, max_tokens=1024, retries=1)
            except Exception:
    ...
    if isinstance(parsed, dict):
        ...
        parsed.setdefault('study_plan', {})
        return parsed

    corrected = _attempt_correction(out)
    if corrected:
        corrected.setdefault('summary', '')
        corrected.setdefault('flashcards', [])
        corrected.setdefault('quiz', [])
        corrected.setdefault('study_plan', {})
        return corrected
